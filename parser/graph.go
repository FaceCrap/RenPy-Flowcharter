...
// DO NOT UNDER ANY CIRCUMSTANCE REPLACE CHARACTERS WITH SPACES
// makes it way too difficult to find the correct label in source code
...
// SAME FOR ANY UNICODE CHARACTERS, LEAVE LABELNAME AS-IS
// because otherwise we have no idea what the original name was
...
func (g *RenpyGraph) AddNode(tags Tag, label string) {
	...
	if tags.title {
		...
		g.nodes[label].repr.Attrs("color", "red", "style", "bold", "shape", "hexagon")
	} else if g.nodes[label].screen {
		g.nodes[label].repr.Attrs("color", "blue", "style", "bold", "shape", "invhouse")
	}
