...
- Activate debug mode, and good luck (or send it to the developer)
	- From a terminal, run 'Flowcharter . -debug'
	- OR Set the debug parameter to 'true' in the 'Flowcharter.config'
...
In the meantime, you'll see an incomplete version of your graph displayed.`)
	...
	if errors.Is(err, ErrorParentNotFound) {
		fmt.Println(`No parent label/screen found.
...
label parent: # Flowcharter: IGNORE
    jump somewhere

- a typo:
label thing # <-- the ':' symbol is missing

- unindented text that does not belong to a label:
label parent:
    "bla blah"
    "bla blah"
