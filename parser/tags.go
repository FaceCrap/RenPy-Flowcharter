...
type Tag struct {
	ignore            bool // Flowcharter: IGNORE tag
	title             bool // Flowcharter: TITLE tag
	breakFlow         bool // Flowcharter: BREAK tag
	lowLink           bool // style for implicit jumps
	nestedLabel       bool // style for nested labels
	callLink          bool // style for call statement
	gameOver          bool // Flowcharter: GAMEOVER tag
	skipLink          bool // Flowcharter: SKIPLINK tag
	inGameLabel       bool // Flowcharter: INGAME_LABEL(i, label_name) tag
	inGameJump        bool // Flowcharter: INGAME_JUMP(i, to_label) tag
	inGameIndent      int  // Flowcharter: INGAME indent tag
	fakeLabel         bool // Flowcharter: FAKE_LABEL(label_name) tag
	fakeJump          bool // Flowcharter: FAKE_JUMP(from_label, to_label) tag
...
func (context *Context) handleTags(line string, detect customRegexes) error {
	...
	if strings.Contains(line, "Flowcharter") {
		lineStrings := strings.Split(line, "Flowcharter")
		endOfLine := strings.Join(lineStrings[1:], " ")                   // removes everything before `Flowcharter`
